...
#
class AuthHMAC
  include AuthHmac::Headers
  @@default_signature_class = AuthHmac::CanonicalString
  ...
  class << self
    ...
    # signature method.
    # 
    def canonical_string(request, options = nil)
      new(nil, options).canonical_string(request)
    end
    ...
    # signature method.
    # 
    def authenticated?(request, access_key_id, secret, options)
      credentials = { access_key_id => secret }
      new(credentials, options).authenticated?(request)
    end
  end
  ...
  def sign!(request, access_key_id)
    ...
    request['WWW-Authenticate'] = authentication(request, access_key_id, secret)
  end
  ...
  def authenticated?(request)
    ...
    if md = rx.match(authentication_header(request))
      access_key_id = md[1]
      hmac = md[2]
      secret = @credential_store[access_key_id]
  ...
  def authentication_header(request)
    find_header(%w(WWW-Authenticate HTTP_WWW_AUTHENTICATE), headers(request))
  end

  def authentication(request, access_key_id, secret)
