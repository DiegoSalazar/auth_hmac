...
describe AuthHMAC do
  ...
  describe ".sign!" do
    ...
    it "should sign using the key passed in as a parameter" do
     ...
     @request['WWW-Authenticate'].should == "AuthHMAC my-key-id:71wAJM4IIu/3o6lcqx/tw7XnAJs="
    end

    it "should sign using custom service id" do
      AuthHMAC.sign!(@request, "my-key-id", "secret", { :service_id => 'MyService' })
      @request['WWW-Authenticate'].should == "MyService my-key-id:71wAJM4IIu/3o6lcqx/tw7XnAJs="
    end
    ...
    it "should sign using custom signature method" do
      ...
      @request['WWW-Authenticate'].should == "MyService my-key-id:/L4N1v1BZSHfAYkQjsvZn696D9c="
    end
  end
  ...
  describe "#sign!" do
    ...
    describe "default AuthHMAC with CanonicalString signature" do
      it "should add an WWW-Authenticate header" do
        @authhmac.sign!(@get_request, 'key-id')
        @get_request.key?("WWW-Authenticate").should be_true
      end
      ...
      it "should create a complete signature" do
        ...
        @put_request['WWW-Authenticate'].should == "AuthHMAC my-key-id:71wAJM4IIu/3o6lcqx/tw7XnAJs="
      end
    end
    ...
    describe "custom signatures" do
      ...
      it "should prefix the Authorization header with custom service id" do
        @authhmac.sign!(@get_request, 'key-id')
        @get_request['WWW-Authenticate'].should match(/^MyService /)
      end
      
      it "should create a complete signature using options" do
        @store.should_receive(:[]).with('my-key-id').and_return('secret')
        @authhmac.sign!(@put_request, "my-key-id")
        @put_request['WWW-Authenticate'].should == "MyService my-key-id:/L4N1v1BZSHfAYkQjsvZn696D9c="
      end
    end
  end
  ...
  describe "authenticated?" do
    ...
    it "should return false when there is no WWW-Authenticate Header" do
      @authhmac.authenticated?(@request).should be_false
    end
    
    it "should return false when the WWW-Authenticate value isn't prefixed with HMAC" do
      @request['WWW-Authenticate'] = "id:secret"
      @authhmac.authenticated?(@request).should be_false
    end
    
    it "should return false when the access key id can't be found" do
      @request['WWW-Authenticate'] = 'AuthHMAC missing-key:blah'
      @authhmac.authenticated?(@request).should be_false
    end    
    
    it "should return false when there is no hmac" do
      @request['WWW-Authenticate'] = 'AuthHMAC missing-key:'
      @authhmac.authenticated?(@request).should be_false
    end
    
    it "should return false when the hmac doesn't match" do
      @request['WWW-Authenticate'] = 'AuthHMAC access key 1:blah'
      @authhmac.authenticated?(@request).should be_false
    end
  ...
  describe "#sign! with YAML credentials" do
    ...
    it "should sign with the secret" do
      @authhmac.sign!(@request, "access key 1")
      @request['WWW-Authenticate'].should == "AuthHMAC access key 1:ovwO0OBERuF3/uR3aowaUCkFMiE="
    end
    
    it "should sign with the other secret" do
      @authhmac.sign!(@request, "access key 2")
      @request['WWW-Authenticate'].should == "AuthHMAC access key 2:vT010RQm4IZ6+UCVpK2/N0FLpLw="
    end
  end
